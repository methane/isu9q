...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		...
		err := tx.SelectContext(ctx, &items,
			...
			user.ID, createdAtTime, createdAtTime, itemID,
			user.ID, createdAtTime, createdAtTime, itemID,
...
func postItemEdit(w http.ResponseWriter, r *http.Request) {
	...
	_, err = dbx.ExecContext(r.Context(), "UPDATE `items` SET `price` = ?, `updated_at` = ? WHERE `id` = ?",
		price,
		time.Now(),
		itemID,
	)
...
func postBuy(w http.ResponseWriter, r *http.Request) {
	...
	result, err := dbx.ExecContext(r.Context(), "INSERT INTO `transaction_evidences` (`seller_id`, `buyer_id`, `status`, `item_id`, `item_name`, `item_price`, `item_description`,`item_category_id`,`item_root_category_id`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
		targetItem.SellerID,
		buyer.ID,
	...
	_, err = dbx.ExecContext(r.Context(), "UPDATE `items` SET `buyer_id` = ?, `status` = ?, `updated_at` = ? WHERE `id` = ?",
		buyer.ID,
		ItemStatusTrading,
		time.Now(),
		targetItem.ID,
	...
	go func() {
		...
		scr, serr = APIShipmentCreate(r.Context(), getShipmentServiceURL(), &APIShipmentCreateReq{
	...
	_, err = dbx.ExecContext(r.Context(), "INSERT INTO `shippings` (`transaction_evidence_id`, `status`, `item_name`, `item_id`, `reserve_id`, `reserve_time`, `to_address`, `to_name`, `from_address`, `from_name`, `img_binary`) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
		transactionEvidenceID,
...
func postShip(w http.ResponseWriter, r *http.Request) {
	...
	item := Item{}
	err = tx.GetContext(r.Context(), &item, "SELECT * FROM `items` WHERE `id` = ?", itemID)
	if err == sql.ErrNoRows {
		outputErrorMsg(w, http.StatusNotFound, "item not found")
		tx.Rollback()
		return
	...
	_, err = dbx.ExecContext(r.Context(), "UPDATE `shippings` SET `status` = ?, `img_binary` = ?, `updated_at` = ? WHERE `transaction_evidence_id` = ?",
...
func postShipDone(w http.ResponseWriter, r *http.Request) {
	...
	_, err = dbx.ExecContext(r.Context(), "UPDATE `shippings` SET `status` = ?, `updated_at` = ? WHERE `transaction_evidence_id` = ?",
		ssr.Status,
		time.Now(),
		transactionEvidence.ID,
	)
	...
	_, err = dbx.ExecContext(r.Context(), "UPDATE `transaction_evidences` SET `status` = ?, `updated_at` = ? WHERE `id` = ?",
...
func postComplete(w http.ResponseWriter, r *http.Request) {
	...
	_, err = dbx.ExecContext(r.Context(), "UPDATE `items` SET `status` = ?, `updated_at` = ? WHERE `id` = ?",
		ItemStatusSoldOut,
		time.Now(),
		itemID,
	)
...
func postSell(w http.ResponseWriter, r *http.Request) {
	...
	result, err := dbx.ExecContext(r.Context(), "INSERT INTO `items` (`seller_id`, `status`, `name`, `price`, `description`,`image_name`,`category_id`) VALUES (?, ?, ?, ?, ?, ?, ?)",
		seller.ID,
		ItemStatusOnSale,
		name,
		price,
	...
	now := time.Now()
	_, err = dbx.ExecContext(r.Context(), "UPDATE `users` SET `num_sell_items`=?, `last_bump`=? WHERE `id`=?",
...
func postBump(w http.ResponseWriter, r *http.Request) {
	...
	_, err = dbx.ExecContext(r.Context(), "UPDATE `items` SET `created_at`=?, `updated_at`=? WHERE id=?",
		now,
		now,
		targetItem.ID,
	)
	...
	_, err = dbx.ExecContext(r.Context(), "UPDATE `users` SET `last_bump`=? WHERE id=?",
		now,
		seller.ID,
	)
	if err != nil {
